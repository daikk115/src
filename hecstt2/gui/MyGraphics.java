...
public class MyGraphics extends JFrame {
    ...
    private File f;
    // danh sách đóng tương ứng với value = 3, ds mở tương ứng với value =2, tường ứng với value =1;
    JTextArea textarea;
    ...
    public MyRobot robot = new MyRobot(0, 0); // khởi tạo tọa độ robot, tạo độ điểm ảnh
    ...
    public MyGraphics() {
        ...
        //tạo JPanel để xây dựng maps và khởi tạo mảng Subcell để mô hình hóa maps thành ma trận
        ...
        img = gc.createCompatibleImage(mapconfig.width + 1, mapconfig.height + 1, Transparency.TRANSLUCENT);

        // khởi tạo phần lưu trữ dữ liệu bản đồ
        matrix = new SubCell[mapconfig.numbercolumns][mapconfig.numberrows]; // các sự kiện khi thao tác trên giao diện hoặc load file sẽ tác động đến dữ liệu ma trận này
...
//        map.setBorder(BorderFactory.createLineBorder(Color.green, 2, true));
        ...
        map.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (matrix[i][j].value) {
                    ...
                    repaint(i * mapconfig.cell, j * mapconfig.cell, mapconfig.cell, mapconfig.cell);
                } else {
        ...
        run.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                // gặp lỗi nhấn 2 lần tìm kiếm là lỗi??? xử lý sao và tại sao lại lỗi
                offline.start();
            }
        });
...
//                if(bw == null) return;
    ...
    public void paint(Graphics g) {
        ...
        // gọi đói tượng Offline được khai báo ở đâ
        drawMatrix(g);
        revalidate();
    }
...
//        g2d.fillRect(0, 0, screen.width - 300, screen.height);
        ...
        for (int i = 0; i < mapconfig.numbercolumns; i++) {
            for (int j = 0; j < mapconfig.numberrows; j++) {
                if (!matrix[i][j].value) {
                    ...
                    g2d.fillRect(i * mapconfig.cell, j * mapconfig.cell, mapconfig.cell, mapconfig.cell);
                } else {
        ...
        for (int i = 1; i <= mapconfig.numbercolumns; i++) { // tại sao không +=mapconfig.cell, lý do là để vẽ đủ số lượng lề thay vì i< mapconfig.width đối khi thiếu dòng cuối
            g2d.drawLine(i * mapconfig.cell, 0, i * mapconfig.cell, mapconfig.height);
        }
        for (int i = 0; i <= mapconfig.numberrows; i++) {
            g2d.drawLine(0, i * mapconfig.cell, mapconfig.width, i * mapconfig.cell); // 0 là x, i là y, tọa độ đề các x sang phải, y đi xuống
        }

        // vẽ con robot
        g2d.drawImage(Toolkit.getDefaultToolkit().getImage(robot.fileImage), robot.x + 1, robot.y + 1, mapconfig.cell - 2, mapconfig.cell - 2, null);
...
//        ImageIO.write(img, "png", outputfile);
        ...
        g2.drawImage(img, mapconfig.x, mapconfig.y, mapconfig.width, mapconfig.height, map);
    }
    ...
    public void drawSTC(Graphics2D g2d) {
        for (Edge e : this.listSTC) {
            ...
            g2d.drawLine(e.start.column * mapconfig.cell, e.start.row * mapconfig.cell, e.end.column * mapconfig.cell, e.end.row * mapconfig.cell);
        }
    }
}
